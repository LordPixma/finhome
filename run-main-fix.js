...
async function fixGlobalAdminViaMainEndpoint() {
  try {
    console.log('🔧 Fixing Global Admin via /api/fix-global-admin-database...\n');
    
    const response = await fetch(`${API_BASE}/api/fix-global-admin-database`, {
      method: 'POST',
      headers: {
    ...
    if (data.success) {
      ...
      console.log('Before:', data.data.before);
      ...
      if (loginData.success) {
        ...
        if (payload.isGlobalAdmin) {
          ...
          if (statsData.success) {
            ...
            console.log('The admin dashboard should now load without errors!');
          }
        } else {
      ...
      console.log('❌ Failed to fix global admin user:', data.error?.message);
    }

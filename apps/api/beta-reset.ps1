...
function Show-Counts($tables,[string]$Label) {
    ...
    foreach ($t in $tables) {
        try {
            $resultOutput = Invoke-Wrangler d1 execute $dbName "--command=SELECT COUNT(*) AS c FROM $t" --remote 2>&1
            # Extract JSON from wrangler output (skip wrangler header lines)
            $jsonLines = $resultOutput | Where-Object { $_ -match '^\s*[\{\[]' }
            if ($jsonLines) {
                $resultJson = $jsonLines -join "`n"
                $parsed = $resultJson | ConvertFrom-Json
                $count = $parsed.results[0].c
                Write-Host ("{0,-28} : {1}" -f $t,$count)
            } else {
                Write-Host ("{0,-28} : N/A" -f $t) -ForegroundColor Yellow
            }
        } catch {
            Write-Host ("{0,-28} : ERROR" -f $t) -ForegroundColor Red
            if ($VerboseMode) { 
                Write-Host "Raw output: $resultOutput" -ForegroundColor Gray
...
if (-not $SkipKV) {
    ...
    foreach ($ns in @($kvSessions,$kvCache)) {
        ...
        try {
            $keysOutput = Invoke-Wrangler kv key list --namespace-id $ns 2>&1
            # Extract JSON from wrangler output
            $jsonLines = $keysOutput | Where-Object { $_ -match '^\s*[\{\[]' }
            if ($jsonLines) {
                $keysJson = $jsonLines -join "`n"
                ...
                Write-Host "  No keys found or failed to parse output for $ns" -ForegroundColor Yellow
                if ($VerboseMode) { Write-Host "Raw output: $keysOutput" -ForegroundColor Gray }
            }
        } catch {
            Write-Host "  Failed processing namespace $ns" -ForegroundColor Red
            if ($VerboseMode) { 
                Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Gray
            }
        }
...
if (-not $SkipVerification) {
    ...
    try {
        $adminOutput = Invoke-Wrangler d1 execute $dbName "--command=SELECT id,email,is_global_admin,tenant_id FROM users WHERE email='admin@finhome360.com'" --remote 2>&1
        # Extract JSON from wrangler output
        $jsonLines = $adminOutput | Where-Object { $_ -match '^\s*[\{\[]' }
        if ($jsonLines) {
            $adminJson = $jsonLines -join "`n"
            ...
            if ($adminParsed.results.Count -eq 1 -and $adminParsed.results[0].is_global_admin -eq 1) {
                ...
                if ($VerboseMode) { Write-Host "Parsed: $adminJson" -ForegroundColor Gray }
            }
        } else {
            Write-Host 'Failed to extract JSON from admin verification output.' -ForegroundColor Red
            if ($VerboseMode) { Write-Host "Raw output: $adminOutput" -ForegroundColor Gray }
        }
    } catch {
        Write-Host 'Failed to parse admin verification output.' -ForegroundColor Red
        if ($VerboseMode) { Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Gray }
    }
}
